...
function applyDynamicDisplayPatterns(value = '') {
  ...
  output = output.replace(/^Prestige Rank:\s*([\d,]+)\s*·\s*Unspent Prestige Points:\s*([\d,]+)$/i, '重修层数：$1 · 未分配重修点：$2')
  output = output.replace(/^Ascension requirement:\s*Level\s+(\d+)\+$/i, '飞升条件：修为等级 $1 以上')
  output = output.replace(/^Estimated ascension reward:\s*Ascending now grants\s+([\d,]+)\s+Prestige Points?\s*\(raw estimate\s+([\d.]+)\)\.\s*Next \+1 point at level\s+([\d,]+)\.$/i, '预计飞升所得：现在飞升可获得 $1 点重修点数（原始估值 $2）；下一点将在修为等级 $3 获得。')
  output = output.replace(/^Estimated ascension reward:\s*Reach (?:level|修为等级)\s+(\d+)\s+to earn 1 (?:Prestige|重修) Point\.$/i, '预计飞升所得：修为等级达到 $1 后可获得 1 点重修点数。')
  output = output.replace(/^Current bonuses:\s*\+([\d.-]+)% XP\s*·\s*\+([\d.-]+)% Gold\s*·\s*\+([\d.-]+)% Loot\s*·\s*\+([\d.-]+)% Damage\s*·\s*([\d.-]+)% Incoming$/i, '当前加成：修为 +$1% · 灵石 +$2% · 战利品 +$3% · 造成伤害 +$4% · 承受伤害 $5%')
  output = output.replace(/^Class:\s*(.+?)\s*·\s*(Simulator Points|Unspent Points):\s*([\d,]+)\s*·\s*Legacy Ranks:\s*([\d,]+)\s*·\s*Unlocks at level\s*(\d+)\+$/i, (_match, cls, pointMode, points, ranks, level) => `${pointMode === 'Simulator Points' ? '推演点数' : '未分配悟道点'}：${points} · 初始传承：${cls} · 无尽悟道层数：${ranks} · 修为等级 ${level} 开启`)
  output = output.replace(/^AA XP Progress:\s*([\d,]+)\/([\d,]+)\s*·\s*Next point in\s*([\d,]+)\s*AA XP$/i, '悟道修为进度：$1/$2 · 距下一点还需 $3 点悟道修为')
  output = output.replace(/^XP to AA:\s*(\d+)%$/i, '修为转入悟道：$1%')
  output = output.replace(/^Regular XP receives\s*(\d+)% of earned XP once level\s*(\d+)\+ is reached\.$/i, '修为等级达到 $2 后，常规修为获得所得修为的 $1%。')
  output = output.replace(/^(.+?)\s*·\s*Rank\s+([\d,]+)$/i, '$1 · 层数 $2')
  output = output.replace(/^·\s*Rank\s+([\d,]+)$/i, '· 层数 $1')
  output = output.replace(/^Current:\s*(.+?)\s*·\s*Next:\s*(.+)$/i, '当前：$1 · 下一层：$2')
  output = output.replace(/^Invest\s+([\d,]+)\s+AA$/i, '投入 $1 点悟道')
  output = output.replace(/^Invest\s+1\s+Prestige$/i, '投入 1 点重修')
  output = output.replace(/^Ascend\s*\((?:Lv|等级|修为等级)\s*(\d+)\+\)$/i, '飞升（修为等级 $1 以上）')
  output = output.replace(/^Respec Perks\s*\(([\d,]+)g\)$/i, '重置重修加点（$1 灵石）')
  output = output.replace(/^Respec\s*\(([\d,]+)g\)$/i, '重置悟道（$1 灵石）')
  output = output.replace(/^Permanent ranks with escalating costs and diminishing percentage gains\. Legacy AA is not refunded by respecs or changed by presets\. Total invested:\s*([\d,]+)\s*AA\.$/i, '层数可无限提升，但消耗会逐渐增加、百分比收益会逐渐衰减。重置悟道或切换预设不会返还或改变无尽悟道。累计投入：$1 点悟道。')
  output = output.replace(/^Next Rank:\s*(.+)$/i, '下一层：$1')
  output = output.replace(/^Requires:\s*(.+)$/i, '需要：$1')
  output = output.replace(/^\[(\d+)\s+pts\]$/i, '[$1 点]')
  output = output.replace(/^Snapshots:\s*A\s+([\d,]+)\s+pts\s*·\s*B\s+([\d,]+)\s+pts$/i, '推演快照：甲 $1 点 · 乙 $2 点')
  output = output.replace(/^Live:\s*(.+)$/i, '当前方案：$1')
  output = output.replace(/^Sim A:\s*(.+)$/i, '推演甲：$1')
  output = output.replace(/^Sim B:\s*(.+)$/i, '推演乙：$1')
  output = output.replace(/^Unlocks at level\s+(\d+)\. Locked in until you pay to respec\.$/i, '修为等级达到 $1 后开启；选定后需消耗灵石才能重置。')
  output = output.replace(/^Respec Specialization\s*\(([\d,]+)g\)$/i, '重置传承专精（$1 灵石）')
  output = output.replace(/^AA is locked until level\s+(\d+)\. Your allocation is saved now and activates automatically at unlock\.$/i, '悟道将在修为等级 $1 开启；当前分配比例会保留，并在开启后自动生效。')
